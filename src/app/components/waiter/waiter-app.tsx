  ...
  return (
    <div className="min-h-screen bg-background flex flex-col max-w-7xl mx-auto shadow-2xl border-x border-border">
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        <Routes>
          <Route index element={<WaiterDashboard tables={tables} loading={loading} fetchTables={fetchTables} onTableClick={(table) => navigate(`ordering/${table.id}`)} />} />
          <Route path="dashboard" element={<WaiterDashboard tables={tables} loading={loading} fetchTables={fetchTables} onTableClick={(table) => navigate(`ordering/${table.id}`)} />} />
          <Route path="ordering/:tableId" element={<WaiterOrdering menuItems={menuItems} onBack={() => navigate('/waiter/dashboard')} />} />
...
function WaiterDashboard({ tables, loading, fetchTables, onTableClick }: { tables: RestaurantTable[], loading: boolean, fetchTables: () => void, onTableClick: (table: RestaurantTable) => void }) {
  return (
